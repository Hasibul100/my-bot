...
(function () {
  ...
  let candleWaitAnimId = null;
  let candleWaitBackupInterval = null;
  ...
  async function getMarketAnalysisSignal(forceFresh) {
    if (!settings.apiKey || !settings.tradingPair) {
      return { direction: null, confirmed: false, source: "no-api-key" };
    }
    const now = Date.now();
    if (!forceFresh && analysisCache.timestamp && now - analysisCache.timestamp < 8000) {
  ...
  // ১-মিনিট running candle বন্ধ (close) হওয়ার ঠিক মুহূর্ত পর্যন্ত অপেক্ষা করে — pending
  // scheduler-এর মতোই platform ঘড়ির সাথে ক্যালিব্রেট করে মিলিসেকেন্ড-নির্ভুলভাবে (২৫০ms
  // বাফার দেওয়া হয় candle close-এর ডেটা API-তে propagate হওয়ার সময়ের জন্য)।
  function waitForNextCandleClose() {
    return new Promise((resolve) => {
      calibratePlatformOffset((offsetMs) => {
        const nowAdjusted = new Date(Date.now() + offsetMs);
        const msIntoMinute = nowAdjusted.getSeconds() * 1000 + nowAdjusted.getMilliseconds();
        const targetTimestamp = Date.now() + (60000 - msIntoMinute) + 250;

        function check() {
          if (!settings.analysisMode || Date.now() >= targetTimestamp) {
            if (candleWaitAnimId) cancelAnimationFrame(candleWaitAnimId);
            if (candleWaitBackupInterval) clearInterval(candleWaitBackupInterval);
            candleWaitAnimId = null;
            candleWaitBackupInterval = null;
            resolve();
            return;
          }
          candleWaitAnimId = requestAnimationFrame(check);
        }
        candleWaitAnimId = requestAnimationFrame(check);
        candleWaitBackupInterval = setInterval(check, 50);
      }, 1500);
    });
  }

  // === "প্রতি ৩ সেকেন্ডে ট্রেড খোলা"-র ফিক্স ===
  // আগে প্রতি ৩s পোলিং করে যেকোনো মুহূর্তে confirmed পেলেই ট্রেড করে ফেলতো — ফলে একই
  // running candle বন্ধ হওয়ার আগেই একাধিকবার ট্রেড হয়ে যাচ্ছিল। এখন থেকে (সব strategy-র
  // ক্ষেত্রেই) running candle বন্ধ না হওয়া পর্যন্ত কোনো ট্রেড হয় না; বন্ধ হওয়ার সাথে সাথে
  // ফ্রেশ ডেটা এনে একবার মূল্যায়ন করা হয় — কনফার্ম হলে তখনই ট্রেড, নাহলে পরের candle
  // বন্ধ হওয়া পর্যন্ত আবার অপেক্ষা।
  async function waitForConfirmedSignal(maxWaitMs) {
    ...
    while (settings.analysisMode) {
      ...
      flashWidgetLabel("WAIT CANDLE", "#ffb020");
      await waitForNextCandleClose();
      if (!settings.analysisMode) return null;
      if (Date.now() - startedAt >= maxWaitMs) return null;

      const result = await getMarketAnalysisSignal(true); // ক্যাশ বাদ দিয়ে ফ্রেশ candle পড়ি
  ...
  window.__HRTrader_STOP__ = function () {
    ...
    resetTapCounter();
    closePanel();
    settings.analysisMode = false; // চলমান waitForConfirmedSignal/candle-close লুপ থামাতে
    if (shutdownTimerTimeout) clearTimeout(shutdownTimerTimeout);
    if (pendingAnimationId) cancelAnimationFrame(pendingAnimationId);
    if (pendingBackupInterval) clearInterval(pendingBackupInterval);
    if (candleWaitAnimId) cancelAnimationFrame(candleWaitAnimId);
