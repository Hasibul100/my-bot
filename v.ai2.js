...
(function () {
  ...
  panel.innerHTML = `
    ...
    <div class="hrtrader-row">
      ...
      <p style="font-size:10px; color:#9fd4ad; margin:4px 0 0;">EMA+RSI ও pattern একমত না হওয়া পর্যন্ত অপেক্ষা করবে, random নেবে না — এত সময় পার হলে এই ট্রেড স্কিপ হবে।</p>
    </div>
  ...
  async function getMarketAnalysisSignal() {
    ...
    try {
      ...
      let emaSignal = null;
      if (ema9 > ema21 && rsi < 70) emaSignal = "up";
      else if (ema9 < ema21 && rsi > 30) emaSignal = "down";

      const patternSignal = detectCandlePattern(candles);

      // শুধু EMA+RSI আর pattern দুটো একমত হলেই "confirmed" — এটাই সবচেয়ে শক্তিশালী
      // সিগনাল যা এই সেটআপ দিতে পারে, বাকি সব ক্ষেত্রে অপেক্ষা করা হবে।
      let direction = null;
      let confirmed = false;
      let source;
      if (emaSignal && patternSignal && emaSignal === patternSignal) {
        direction = emaSignal;
        confirmed = true;
        source = "EMA+RSI+pattern (confirmed)";
      } else if (emaSignal && patternSignal) {
        source = "conflict — waiting";
      } else if (emaSignal || patternSignal) {
        source = (emaSignal ? "EMA+RSI" : "pattern") + " only — waiting for confirmation";
      } else {
        source = "no signal — waiting";
      }

      console.log(
        `HRTrader analysis [${settings.tradingPair}]: EMA9=${ema9.toFixed(5)} EMA21=${ema21.toFixed(
          5
        )} RSI=${rsi.toFixed(1)} pattern=${patternSignal || "none"} → ${
          direction ? direction.toUpperCase() : "WAIT"
        } (${source})`
      );
      analysisCache = { direction, confirmed, timestamp: now, source };
      return analysisCache;
    } catch (err) {
