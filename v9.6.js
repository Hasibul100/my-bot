...
// ৫. আল্ট্রা-ফাস্ট পেন্ডিং শিডিউলার (০ms লেটেন্সি লজিক)
    ...
    function startPendingScheduler() {
      ...
      console.log(`[HRTrader] Scheduler Armed for ${settings.pendingTime}. Zero latency mode active.`);
      ...
      function checkTimeHighPrecision() {
        ...
        // যখনই টার্গেট ঘণ্টা এবং মিনিট মিলবে, সাথে সাথে কোনো ডিলে ছাড়া এক্সিকিউট হবে
        if (currentHH === targetHH && currentMM === targetMM) {
          // রেস কন্ডিশন এড়াতে প্রথমেই এনিমেশন ফ্রেম ও শিডিউলার বন্ধ করুন
          settings.pendingEnabled = false; 
          saveSettings(settings);
          
          // ইনস্ট্যান্ট ট্রেড ফায়ার (কোনো থার্ড পার্টি ডিলে ছাড়া)
          executePlatformTrade(settings.pendingDir);
          
          syncUI();
          return; // লুপ থেকে বের হয়ে যান
        }

        // ব্রাউজারের হাই-স্পিড গ্রাফিক্স ক্লকের সাথে সিঙ্ক করে প্রতি সেকেন্ডে ৬০+ বার চেক করবে
