/**
 * HRTrader Pro Ultimate v6.0 — Human Keyboard Emulation Protocol (100% Fixed Amount Reset)
...
(function () {
  ...
  function initHRTrader() {
    ...
    /**
     * মাস্টার ফিক্স: আলটিমেট ট্রাস্টেড হিউম্যান টাইপিং ইমুলেটর প্রোটোকল
     */
    async function setPlatformTradeAmount(amount) {
      ...
      try {
        ...
        // ১. পুরোনো টেক্সট সম্পূর্ণ ক্লিয়ার করার জন্য ব্যাকস্পেস এমুলেশন
        amountInputEl.value = "";
        amountInputEl.dispatchEvent(new Event('input', { bubbles: true }));

        // ২. মানুষের টাইপিং স্পিডে এক একটি ক্যারেক্টার পুশ করা
        let amtStr = String(amount);
        for (let char of amtStr) {
          let keyEventOpts = { key: char, keyCode: char.charCodeAt(0), bubbles: true, cancelable: true };
          
          amountInputEl.dispatchEvent(new KeyboardEvent('keydown', keyEventOpts));
          amountInputEl.dispatchEvent(new KeyboardEvent('keypress', keyEventOpts));
          
          amountInputEl.value += char;
          
          amountInputEl.dispatchEvent(new Event('input', { bubbles: true }));
          amountInputEl.dispatchEvent(new Event('change', { bubbles: true }));
          amountInputEl.dispatchEvent(new KeyboardEvent('keyup', keyEventOpts));
        }

        // ৩. ফাইনাল ব্লার সিঙ্ক রিঅ্যাক্ট স্টেট লক করার জন্য
        amountInputEl.dispatchEvent(new Event('blur', { bubbles: true }));
        console.log(`[HRTrader] Target Amount Simulated & Hard Locked To: $${amount}`);
      } catch (err) {
        console.error("Trusted typing system failed, using hard fallback: ", err);
    ...
    async function executePlatformTrade(customDir = null, customAmount = null, skipAmountSetup = false) {
      ...
      if (!skipAmountSetup) {
        ...
        await setPlatformTradeAmount(finalAmount);
      }

      // রিঅ্যাক্ট স্টেট আপডেট প্রসেস করার জন্য ৩০০ মিলিসেকেন্ড ওয়েট গ্যাপ
      setTimeout(() => {
        ...
        try {
          ...
          console.log(`[HRTrader] Active Execution Success on: ${finalDir}`);
        } catch(e) {
    ...
    function startPendingScheduler() {
      ...
      pendingCheckInterval = setInterval(async () => {
        ...
        // ট্রাস্টেড টাইপিং ইমুলেশন একটু সময় নেয়, তাই ৪ সেকেন্ড আগে ফায়ারিং শুরু হবে
        if (currentHHMM === settings.pendingTime && ss >= 56 && !preloadedAmountFlag) {
          preloadedAmountFlag = true; 
          await setPlatformTradeAmount(settings.pendingAmount);
        }
    ...
    setTimeout(() => { setPlatformTradeAmount(settings.tradeAmount); }, 1200);

    console.log("HRTrader Ultimate v6.0 Core-Typing Simulation Active!");
  }
