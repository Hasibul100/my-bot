...
(function () {
  ...
  function initHRTrader() {
    ...
    // স্ক্যান ব্যর্থ হলে (payout/button not found) ইউজারকে বোঝানোর জন্য
    // সংক্ষিপ্ত সময়ের জন্য উইজেটের লেবেলে কারণ দেখানো হয়, শুধু console.warn না।
    ...
    function getCurrentPayoutPercent() {
      ...
      // আগে এখানে সিলেক্টর না পেলে চুপচাপ 80 রিটার্ন করতো, যার ফলে
      // minPayout ফিল্টার অকার্যকর হয়ে যেত (আসল পেআউট না জেনেই ট্রেড চলতো)।
      // এখন null রিটার্ন করি যাতে caller বুঝতে পারে পেআউট আনরিলায়েবল।
      return null;
    }
    ...
    function startBalanceTracker() {
      ...
      balanceCheckInterval = setInterval(() => {
        ...
        if (current !== sessionStats.lastBalance) {
          ...
          // আগে শুধু ব্যালেন্স বাড়লে WIN গোনা হতো, কমলে কিছুই হতো না —
          // ফলে LOSS কাউন্ট সবসময় ০ থাকতো এবং isTradeActive আটকে থেকে
          // পরের ট্রেডের রেজাল্টকে ভুল ট্রেডের সাথে জুড়ে দিতো।
          if (current > sessionStats.lastBalance) {
    ...
    function executePlatformTrade(customDir = null) {
      ...
      if (currentPayout === null) {
        console.warn("Payout selector not found — payout unknown, skipping trade for safety.");
        flashWidgetLabel("PAYOUT?", "#ff5252");
        stopScanSession();
        return;
      }
      if (currentPayout < settings.minPayout) {
        console.warn(`Payout too low (${currentPayout}%). Skipped.`);
        flashWidgetLabel(`LOW ${currentPayout}%`, "#ffb020");
        stopScanSession();
        return;
      }
    ...
    function startPendingScheduler() {
      ...
      pendingAnimationId = requestAnimationFrame(checkTimeHighPrecision);
      // মোবাইল ব্রাউজারে ট্যাব ব্যাকগ্রাউন্ডে গেলে requestAnimationFrame থ্রটল/পজ হয়ে যায়,
      // তাই ব্যাকআপ হিসেবে setInterval রাখা হয়েছে যাতে টাইমিং মিস না হয়।
      // isExecuted ফ্ল্যাগ ডাবল-এক্সিকিউশন আটকায়, তাই দুটো একসাথে চলা নিরাপদ।
