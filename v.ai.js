...
(function () {
  ...
  // === Real Market Analysis (Twelve Data API, EMA + RSI) ===
  ...
  async function fetchCandleCloses(symbol, interval, outputsize) {
    ...
    // API নতুন-থেকে-পুরনো ক্রমে ডেটা দেয়, ইন্ডিকেটর হিসাবের জন্য পুরনো-থেকে-নতুন লাগবে
  ...
  async function getMarketAnalysisDirection() {
    if (!settings.apiKey || !settings.tradingPair) {
      console.warn("HRTrader: API key/trading pair সেট করা নেই — random ফলব্যাক।");
      return Math.random() > 0.5 ? "up" : "down";
    }
    const now = Date.now();
    if (analysisCache.direction && now - analysisCache.timestamp < 8000) {
      return analysisCache.direction;
    }
    try {
      const closes = await fetchCandleCloses(settings.tradingPair, "1min", 50);
      const ema9 = calcEMA(closes.slice(-30), 9);
      const ema21 = calcEMA(closes.slice(-30), 21);
      const rsi = calcRSI(closes, 14);

      let direction;
      if (ema9 > ema21 && rsi < 70) direction = "up";
      else if (ema9 < ema21 && rsi > 30) direction = "down";
      else direction = Math.random() > 0.5 ? "up" : "down"; // সংকেত অস্পষ্ট হলে ফলব্যাক

      console.log(
        `HRTrader analysis [${settings.tradingPair}]: EMA9=${ema9.toFixed(5)} EMA21=${ema21.toFixed(
          5
        )} RSI=${rsi.toFixed(1)} → ${direction.toUpperCase()}`
      );
      analysisCache = { direction, timestamp: now };
      return direction;
    } catch (err) {
      console.error("HRTrader: market analysis ব্যর্থ হয়েছে, random এ ফলব্যাক করা হলো।", err);
      flashWidgetLabel("API ERR", "#ff5252");
      return Math.random() > 0.5 ? "up" : "down";
    }
  }
  ...
  async function executeTrade(customDir) {
    ...
    if (!dirOverride && settings.analysisMode) {
      ...
      dirOverride = await getMarketAnalysisDirection();
    }
  ...
  function runAutoTradeCycle() {
    ...
    autoTradeTimeout = setTimeout(async () => {
      await executeTrade();
      const waitMs = getTradeDurationMs() + 2000; // এই ট্রেড শেষ হওয়া পর্যন্ত অপেক্ষা করে পরেরটা শুরু করি
