/**
 * HRTrader Pro Ultimate v5.8 — Bug-Free Core, Live Win/Loss Stats & Payout % Filter
 * Target Selector: .deal-amount-input input.input-control__input
 */
(function () {
  ...
  function initHRTrader() {
    ...
    // লাইভ লাভ লসের উইন/লস কাউন্টার সহ সেশন স্ট্যাটস
    let sessionStats = { 
      tradesCount: 0, 
      winCount: 0, 
      lossCount: 0, 
      currentMartingaleAmount: 1, 
      baseBalance: null, 
      lastBalance: null,
      lastDirection: "up", 
    ...
    function loadSettings() {
      try {
        ...
        if (stored) {
          ...
          // বুট টাইমে অটোমেটিক পেন্ডিং অফ রাখা হবে যেন বাগ না হয়
          parsed.pendingEnabled = false;
          parsed.pendingTime = getSafeFutureTimeHHMM();
    ...
    panel.innerHTML = `
      ...
      <!-- লাইভ লাভ লসের ট্র্যাকার ইন্টারফেস -->
      ...
      <!-- মিনিমাম পেআউট পার্সেন্ট ফিল্টার -->
      <div class="hrtrader-row">
        <label>মিনিমাম পেআউট পার্সেন্ট (%)</label>
        <input id="cfg-min-payout" type="number" min="1" max="100" placeholder="যেমন: 80" />
      </div>
      ...
      <div class="hrtrader-row">
        <label>ট্রেড ওপেন টাইম (HH:MM)</label>
    ...
    function setPlatformTradeAmount(amount) {
      let amountInputEl = document.querySelector(".deal-amount-input input.input-control__input");
      if (amountInputEl) {
    ...
    function saveAll() {
      // সমস্ত ইনপুট ভ্যালু পার্স করে অবজেক্টে স্টোর করা (ফিক্সড টাইপো)
      ...
      // সেভ করার সাথে সাথেই ইনপুটে ভ্যালু পুশ হবে
      setPlatformTradeAmount(settings.tradeAmount);
      
      preloadedAmountFlag = false; 
      startShutdownTimer();
    ...
    // প্ল্যাটফর্মের কারেন্ট পেআউট পার্সেন্টেজ রিড করার ফাংশন
    function getCurrentPayoutPercent() {
      let payoutEl = document.querySelector(".payout-percent, .current-payout, .v2KPX .payout");
      if (!payoutEl) {
        // অল্টারনেটিভ সিলেক্টর ট্রাই
        let allItems = document.querySelectorAll('span, div');
        for (let el of allItems) {
          if (el.textContent.includes('%') && el.textContent.length <= 4) {
            let num = parseInt(el.textContent.replace(/[^0-9]/g, ''));
            if (num > 10 && num <= 100) return num;
          }
        }
      } else {
        return parseInt(payoutEl.textContent.replace(/[^0-9]/g, "")) || 0;
      }
      return 80; // সিলেক্টর না পেলে ডিফল্ট ৮০ ধরে ব্যাকআপ রাখবে
    }

    // লাভ/ক্ষতি ট্র্যাকিং লুপ
    function startBalanceTracker() {
      ...
      balanceCheckInterval = setInterval(() => {
        ...
        if (current > sessionStats.lastBalance) {
          ...
          console.log("HRTrader Result: WIN!");
        } else if (current < sessionStats.lastBalance) {
          sessionStats.lossCount++;
          sessionStats.lastBalance = current;
          console.log("HRTrader Result: LOSS!");
        }
      }, 2000);
    }
    ...
    function executePlatformTrade(customDir = null, customAmount = null, skipAmountSetup = false) {
      if (sessionStats.tradesCount >= settings.maxTrades) {
        stopScanSession();
        return;
      }

      // মিনিমাম পার্সেন্টেজ ফিল্টার রুল চেক
      let currentPayout = getCurrentPayoutPercent();
      if (currentPayout < settings.minPayout) {
        console.warn(`Trade Skipped! Current payout (${currentPayout}%) is lower than minimum setting (${settings.minPayout}%).`);
        stopScanSession();
        return;
      }
    ...
    function startPendingScheduler() {
      ...
      pendingCheckInterval = setInterval(() => {
        // যদি টগল অফ থাকে, তাহলে কিচ্ছু করবে না
        ...
        // ১.৫ সেকেন্ড ল্যাগ কাটার জন্য ৩ সেকেন্ড আগেই ব্যাকগ্রাউন্ডে অ্যামাউন্ট পুশ করা হবে
        if (currentHHMM === settings.pendingTime && ss >= 57 && !preloadedAmountFlag) {
          setPlatformTradeAmount(settings.pendingAmount);
          preloadedAmountFlag = true; 
        }
        
        // এক্সাক্ট ০ সেকেন্ডে জিরো-ল্যাগ ক্লিক ফায়ারিং
        if (currentHHMM === settings.pendingTime && ss === 0) {
          clearInterval(pendingCheckInterval); 
          pendingCheckInterval = null;
          
          executePlatformTrade(settings.pendingDir, settings.pendingAmount, true);
    ...
    function startAutoTrade() {
      ...
      let finalAmount = settings.tradeAmount;
      setPlatformTradeAmount(finalAmount);

      tradeTimer = setTimeout(() => { tradeTimer = null; executePlatformTrade(null, null, true); }, settings.delaySec * 1000);
    }
    ...
    let initInput = document.querySelector(".deal-amount-input input.input-control__input");
    if(initInput) { initInput.value = settings.tradeAmount; }

    console.log("HRTrader Ultimate v9 Ultra Safe Protocol Loaded!");
  }
