/**
 * HRTrader Pro Ultimate v5.7 — Instant Zero-Lag & Dedicated Pending Toggle
...
(function () {
  ...
  function initHRTrader() {
    ...
    const defaults = {
      ...
      pendingAmount: 1,
      pendingDir: "up",
      pendingEnabled: false // নতুন টগল স্টেট
    };
    ...
    function saveAll() {
      ...
      // ডিফল্ট অ্যামাউন্ট আগেই সেট করে রাখছি
    ...
    // skipAmountSetup পাস করলে ডাইরেক্ট জিরো-ল্যাগে ফায়ার হবে
    function executePlatformTrade(customDir = null, customAmount = null, skipAmountSetup = false) {
      ...
      // প্রি-লোড না হয়ে থাকলে কেবল তখনি চেঞ্জ করবে
      if (!skipAmountSetup) {
    ...
    function startPendingScheduler() {
      ...
      pendingCheckInterval = setInterval(() => {
        // যদি টগল অফ থাকে, কিচ্ছু করবে না
        ...
        // ১.৫ সেকেন্ড ল্যাগ কাটার আসল মাস্টারপ্ল্যান:
        // নির্দিষ্ট টাইম হওয়ার ২ সেকেন্ড আগে (যখন ৫৮ সেকেন্ড) ব্যাকগ্রাউন্ডে অ্যামাউন্ট লোড করে রাখা হবে।
    ...
    function startAutoTrade() {
      ...
      // নরমাল ম্যানুয়াল স্ক্যানেও ১.৫ সেকেন্ড ল্যাগ ফিক্স করার জন্য সাথে সাথে অ্যামাউন্ট লোড
      let finalAmount = settings.tradeAmount;
      if (sessionStats.isMartingaleMode) { finalAmount = settings.tradeAmount * sessionStats.currentMartingaleAmount; }
      setPlatformTradeAmount(finalAmount);
    ...
    console.log("HRTrader Ultimate v5.7 Super-Speed Zero-Lag Protocol Loaded!");
  }
