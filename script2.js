...
(function () {
  ...
  // STRICT MULTI-PLATFORM BUTTON DETECTOR
  function getTradeButtons() {
    // 1. Quotex Target (আপনার দেওয়া সুনির্দিষ্ট ক্লাসের ওপর ভিত্তি করে)
    ...
    // 2. Pocket Option Target Fallback (যদি কোটেক্সের বাটন না পাওয়া যায়)
    if (!upBtn || !downBtn) {
      upBtn = document.querySelector('.control__up .btn, [data-action="call"], .btn-call, .divider-vertical[data-type="up"]');
      downBtn = document.querySelector('.control__down .btn, [data-action="put"], .btn-put, .divider-vertical[data-type="down"]');
    }
  ...
  function placeTrade() {
    ...
    if (!btn) {
      console.error("HRTrader: target button not found.");
      stopScanSession();
      return false;
    }

    // TRIGGER BROWSER CLICK FOR QUOTEX EXCLUSIVE CLASSES
    try {
      btn.click();
      
      // ডাবল অ্যাকশন ট্রিগার
      ['mousedown', 'mouseup'].forEach(evtName => {
        const ev = new MouseEvent(evtName, { bubbles: true, cancelable: true, view: window });
        btn.dispatchEvent(ev);
      });
    } catch(e) {
      console.error("HRTrader Click Error:", e);
    }
    
    // ট্রেডের দিক নির্ধারণ লজিক
    const isUp = btn.classList.contains('JQZcs') || btn.classList.contains('btn-call') || (btn.closest && btn.closest('.control__up'));
  ...
  syncPanelUI();
  console.log(
    "HRTrader Quotex Fixed Loaded — Ready to Trade!"
  );
  }
