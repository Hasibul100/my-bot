/**
 * HRTrader Pro v9.4 — Anti-Freeze Toggle Edition (Pure Click Execution)
 * Fixes the scheduler lockup/freeze after toggling the pending switch.
 */
(function () {
  ...
  function initHRTrader() {
    ...
    panel.innerHTML = `
      <h3>HRTrader Robot Dashboard (v9.4)</h3>
    ...
    function saveAll() {
      ...
      startShutdownTimer();
      startPendingScheduler(); // মাস্টার ফিক্স: টগল সেভ করার সাথে সাথে ফ্রেশ লুপ স্টার্ট হবে
      closePanel();
    }
    ...
    function executePlatformTrade(customDir = null) {
      ...
      try {
        ...
        console.log(`[HRTrader] Pure Click Success on: ${finalDir} (Manual amount saved)`);
      } catch(e) {
    ...
    function startPendingScheduler() {
      // মাস্টার ফিক্স: যেকোনো পুরোনো লুপ আগে চিরতরে স্টপ করা হবে যাতে হ্যাং না হয়
      ...
      pendingCheckInterval = setInterval(() => {
        ...
        if (currentHHMM === settings.pendingTime && ss === 0) {
          ...
          // এক্সিকিউশন শেষে UI রিসেটের জন্য সেভ রান
          syncUI();
        }
      }, 500);
    }
    ...
    console.log("HRTrader Anti-Freeze Toggle v9.4 Active!");
  }
