/**
 * HRTrader Pro Ultimate v5.9 — Deep React State Forcer & Anti-Reset Protocol
...
(function () {
  ...
  function initHRTrader() {
    ...
    /**
     * মাস্টার ফিক্স: প্ল্যাটফর্মের রিঅ্যাক্ট/ভিইউ ডম স্টেট ফোর্স করার জন্য প্রোপার্টি ডিসক্রিপ্টর ইনজেকশন
     */
    function setPlatformTradeAmount(amount) {
      ...
      if (amountInputEl) {
        try {
          let lastValue = amountInputEl.value;
          amountInputEl.value = amount;
          
          // ব্রাউজারের ডিপ ভ্যালু সেটার ট্রিগার (যাতে আগের ভ্যালুতে রিসেট না হয়)
          let nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
          nativeInputValueSetter.call(amountInputEl, amount);
          
          // ইনপুট ইভেন্ট বাবলিং ফোর্স
          amountInputEl.dispatchEvent(new Event('input', { bubbles: true }));
          amountInputEl.dispatchEvent(new Event('change', { bubbles: true }));
          
          console.log(`HRTrader Amount Synced Deeply: $${amount}`);
        } catch(e) {
          // ফলব্যাক সেটার পদ্ধতি
          amountInputEl.value = amount;
          amountInputEl.dispatchEvent(new Event('input', { bubbles: true }));
        }
      }
    ...
    function saveAll() {
      ...
      // সেভ করার সাথে সাথে ফোর্সড লক করা হবে
    ...
    function executePlatformTrade(customDir = null, customAmount = null, skipAmountSetup = false) {
      ...
      if (currentPayout < settings.minPayout) {
        console.warn(`Payout too low (${currentPayout}%). Target: ${settings.minPayout}%`);
        stopScanSession();
        return;
      }
      ...
      if (!up || !down) {
        console.warn("Trade buttons not found on screen!");
        stopScanSession();
        return;
      }
      ...
      try {
        ...
        console.log(`HRTrader Trade Placed Successfully! Direction: ${finalDir}`);
      } catch(e) {
    ...
    function startPendingScheduler() {
      ...
      pendingCheckInterval = setInterval(() => {
        ...
        // ২ সেকেন্ড আগে এমাউন্ট ডিপ ফোর্স করা হবে
        if (currentHHMM === settings.pendingTime && ss >= 58 && !preloadedAmountFlag) {
    ...
    // বুট টাইমে কারেন্ট এমাউন্ট পুশ
    setTimeout(() => { setPlatformTradeAmount(settings.tradeAmount); }, 1000);

    console.log("HRTrader Ultimate v5.9 Deep Forced Protocol Loaded!");
  }
