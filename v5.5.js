/**
 * HRTrader Pro Ultimate v5.5 — Absolute Real-Time 100ms Fix & Zero-Start Edition
...
(function () {
  ...
  function initHRTrader() {
    ...
    function loadSettings() {
      try {
        ...
        if (stored) {
          ...
          if (!parsed.pendingTime) parsed.pendingTime = getCurrentTimeHHMM();
          return { ...defaults, ...parsed };
        }
      } catch {}
      return { ...defaults, pendingTime: getCurrentTimeHHMM() };
    }
    ...
    panel.innerHTML = `
      ...
      <h4>পেন্ডিং ট্রেড শিডিউলার (মিনিট কন্ট্রোল)</h4>
    ...
    function syncUI() {
      ...
      pendingTimeInput.value = getCurrentTimeHHMM();
    ...
    function saveAll() {
      ...
      // ফিক্সড: প্ল্যাটফর্ম ইনপুট শুধুমাত্র ভ্যালু পুশ করবে, কোনো রিয়েল ট্রিগার বা বাটন ইভেন্ট রান করবে না।
      let targetInput = document.querySelector(".deal-amount-input input.input-control__input");
      if(targetInput) { targetInput.value = settings.tradeAmount; }
    ...
    function startPendingScheduler() {
      ...
      // ফিক্সড: টাইমার ইন্টারভ্যাল ১০০০ থেকে কমিয়ে ১০০ মিলিসেকেন্ডে আনা হলো, যেন ঘড়ির টাইম আসার ১ পলকেই ট্রেড প্লেস হয়।
      pendingCheckInterval = setInterval(() => {
        ...
        if (currentHHMM === settings.pendingTime) {
          executePlatformTrade(settings.pendingDir, settings.pendingAmount);
          settings.pendingTime = ""; 
          document.querySelector("#cfg-pending-time").value = getCurrentTimeHHMM();
          saveSettings(settings);
        }
      }, 100);
    }
    ...
    // ফিক্সড: বুট হওয়ার সময় ইনপুট ফিল্ডে কোনো ভুল ইভেন্ট ফায়ার করা যাবে না
    ...
    console.log("HRTrader Ultimate v5.5 Absolute Real-Time Fixed!");
  }
