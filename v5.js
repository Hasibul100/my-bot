/**
 * HRTrader Pro Ultimate v4 — Robot Edition
 * Features: Martingale Same Direction, Custom Trade Amount Selector, Pending Trade Scheduler, and Auto-Shutdown Timer.
 */
(function () {
  ...
  function initHRTrader() {
    ...
    const STORAGE_KEY = "hrtrader_pro_settings_v4";
    ...
    function syncUI() {
      ...
      panel.querySelector("#stat-martingale").textContent = sessionStats.currentMartingaleAmount.toFixed(0) + "x";
    }
    ...
    // ব্রোকার ডম ডিটেকশন ফাংশনসমূহ[cite: 1]
    ...
    // ব্রোকারের ম্যানুয়াল ইনপুট বক্সে ডলার অ্যামাউন্ট পুশ করার লজিক
    ...
    if (amountInputEl) {
      ...
      // ব্রোকারের রিয়েল-টাইম স্ক্রিপ্ট যেন ভ্যালু চেঞ্জ ধরতে পারে, তাই ইভেন্ট ফায়ার করা হলো
      amountInputEl.dispatchEvent(new Event('input', { bubbles: true }));
      amountInputEl.dispatchEvent(new Event('change', { bubbles: true }));
      console.log(`🤖 প্ল্যাটফর্মে ট্রেড অ্যামাউন্ট সেট করা হয়েছে: ${amount}$`);
        } else {
      console.error("❌ অ্যামাউন্ট ইনপুট বক্সের ক্লাস নেম খুঁজে পাওয়া যায়নি!");
      }
    }

    function executePlatformTrade(customDir = null, customAmount = null) {
      if (sessionStats.tradesCount >= settings.maxTrades) {
        console.warn("HRTrader: ম্যাক্সিমাম ট্রেড লিমিট শেষ!");
        stopScanSession();
        return;
      }
      ...
      if (currentBalance !== null) {
        ...
        if (diff >= settings.tpAmount) {
          console.log("🔥 টেক প্রফিট ডান! রোবট হোল্ড পজিশন।");
          stopScanSession();
          return;
        }
        if (Math.abs(diff) >= settings.slAmount && diff < 0) {
          console.warn("🛑 স্টপ লস ক্রসড! ক্যাশ প্রোটেকশন মোড অ্যাক্টিভেটেড।");
          stopScanSession();
          return;
        }
      }
      ...
      if (!up || !down) {
        console.error("HRTrader Error: ব্রোকার বাটন ডিটেক্ট করা যাচ্ছে না!");
        stopScanSession();
        return;
      }
      ...
      // অ্যামাউন্ট ডিটারমিনেশন (মার্টিনগেল নাকি নরমাল)
      ...
      // ব্রোকার ইন্টারফেসে অ্যামাউন্ট পুশ করা
      ...
      try {
        ...
        sessionStats.lastDirection = finalDir; // পরবর্তী মার্টিনগেলের জন্য সেভ
        console.log(`HRTrader Trade Placed: ${finalAmount}$, Direction: ${finalDir.toUpperCase()}`);

        // একই ডিরেকশনে মার্টিনগেল লজিক (ট্রেড ক্লোজ হওয়ার পর ব্যালেন্স চেক)
        let balanceBefore = currentBalance;
        setTimeout(() => {
          let balanceAfter = getAccountBalance();
          if (balanceAfter !== null && balanceBefore !== null) {
            if (balanceAfter < balanceBefore) {
              // লস হয়েছে! পরবর্তী ক্যান্ডেলেই সেম ডিরেকশনে মার্টিনগেল অ্যাক্টিভ
              sessionStats.isMartingaleMode = true;
              sessionStats.currentMartingaleAmount *= settings.martingaleMultiplier;
              console.log(`⚠️ ট্রেড লস! পরবর্তী ক্যান্ডেলে সেম ডিরেকশন (${sessionStats.lastDirection.toUpperCase()}) মার্টিনগেল এক্সিকিউট হচ্ছে...`);
              executePlatformTrade(sessionStats.lastDirection);
            } else {
              // উইন হয়েছে! রিসেট মার্টিনগেল
              sessionStats.isMartingaleMode = false;
              sessionStats.currentMartingaleAmount = 1;
            }
          }
        }, 62000); // ১ মিনিটের ক্যান্ডেল রেজাল্ট চেকের জন্য স্ট্যান্ডার্ড ডিলে বাফার
    ...
    // পেন্ডিং টাইম-বেসড শিডিউলার লুপ
    function startPendingScheduler() {
      ...
      pendingCheckInterval = setInterval(() => {
        ...
        if (timeString === settings.pendingTime) {
          console.log(`🎯 পেন্ডিং টাইম ম্যাচ করেছে (${timeString})! অটো ট্রেড ওপেন হচ্ছে...`);
          executePlatformTrade(settings.pendingDir, settings.pendingAmount);
          settings.pendingTime = ""; // একবার এক্সিকিউট হলে ওটা রিসেট করে দিবে
    ...
    // ইনিশিয়াল মেকানিজম রান
    startShutdownTimer();
    startPendingScheduler();

    console.log("HRTrader Pro Robot Ready — মামা পুরো কন্ট্রোল এবার আপনার হাতে!");
  }
